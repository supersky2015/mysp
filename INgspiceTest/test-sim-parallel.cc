...
int cieq1(register char *p, register char *s);
...
int
ng_getchar1(char* outputreturn, int ident, void* userdata);

int
ng_getstat1(char* outputreturn, int ident, void* userdata);

int
ng_thread_runs1(bool noruns, int ident, void* userdata);

ControlledExit ng_exit1;
SendData ng_data1;
SendInitData ng_initdata1;
...
mutexType rt_cs; // used in ng_thread_runs1()
...
int
cieq1(register char *p, register char *s)
{
    while (*p) {
...
/* mutex initializing */
    ...
    *(++exeptr) = '\0';
    ...
    ret = ((int * (*)(SendChar*, SendStat*, ControlledExit*, SendData*, SendInitData*,
                      BGThreadRunning*, void*)) ngSpice_Init_handle1)(ng_getchar1, ng_getstat1,
                              ng_exit1, NULL, ng_initdata1, ng_thread_runs1, NULL);

    ret = ((int * (*)(SendChar*, SendStat*, ControlledExit*, SendData*, SendInitData*,
                      BGThreadRunning*, void*)) ngSpice_Init_handle2)(ng_getchar1, ng_getstat1,
                              ng_exit1, NULL, ng_initdata1, ng_thread_runs1, NULL);
...
/* search for ngspice libraries 1 - 3, create them by copying, if not already existing */
    ...
    ret = ((int * (*)(SendChar*, SendStat*, ControlledExit*, SendData*, SendInitData*,
                      BGThreadRunning*, void*)) ngSpice_Init_handle1)(ng_getchar1, ng_getstat1,
                              ng_exit1, ng_data1, ng_initdata1, ng_thread_runs1, NULL);
    ...
    ret = ((int * (*)(SendChar*, SendStat*, ControlledExit*, SendData*, SendInitData*,
                      BGThreadRunning*, void*)) ngSpice_Init_handle3)(ng_getchar1, ng_getstat1,
                              ng_exit1, ng_data1, ng_initdata1, ng_thread_runs1, NULL);
...
int
ng_exit1(int exitstatus, bool immediate, bool quitexit, int ident, void* userdata)
{
...
int
ng_data1(pvecvaluesall vdata, int numvecs, int ident, void* userdata)
{
    if (ident == 1)
...
int
ng_initdata1(pvecinfoall intdata, int ident, void* userdata)
{
    int i;
    int vn;
    if (ident == 1) {
        ...
        for (i = 0; i < vn; i++) {
            ...
            if (cieq1(intdata->vecs[i]->vecname, "out1"))
                vecgetnumber1 = i;
        }
    } else if  (ident == 2) {
        vn = intdata->veccount;
        for (i = 0; i < vn; i++) {
            printf("Vector: %s\n", intdata->vecs[i]->vecname);
            /* find the location of V(out) */
            if (cieq1(intdata->vecs[i]->vecname, "out2"))
                vecgetnumber2 = i;
        }
    }
    return 0;
